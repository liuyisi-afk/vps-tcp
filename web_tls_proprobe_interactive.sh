#!/usr/bin/env bash
# 交互式 TLS/HTTP2/X25519/OCSP/Hybrid(KEM)/证书长度 探测
...
TLS13_RAW=$(openssl s_client -connect "${HOST}:${PORT}" -servername "${SNI}" -tls1_3 2>&1 || true)
...
check_h2() {
  ...
  out=$(openssl s_client -connect "${HOST}:${PORT}" -servername "${SNI}" ${flag} -alpn h2 2>&1 || true)

  if echo "$out" | grep -Eiq 'ALPN protocol: *h2'; then
    echo "h2(${label} ALPN): YES"
    echo "$out" | sed -n '1,14p'
    return 0
  fi

  if echo "$out" | grep -Eiq 'Next protocol|NPN.*h2'; then
    echo "h2(${label} NPN): YES"
    echo "$out" | sed -n '1,14p'
    return 0
  fi
...
if check_h2 -tls1_3 "TLS1.3"; then
  H2_TLS13_RESULT="支持"
  H2_ANY=0
else
  H2_TLS13_RESULT="未协商"
...
if check_h2 -tls1_2 "TLS1.2"; then
  H2_TLS12_RESULT="支持"
  H2_ANY=0
else
  H2_TLS12_RESULT="未协商"
...
if [[ "$PREFERRED_PROTO" == "-tls1_3" ]]; then
  X25519_RAW=$(openssl s_client -connect "${HOST}:${PORT}" -servername "${SNI}" -tls1_3 -groups X25519 2>&1 || true)
else
  X25519_RAW=$(openssl s_client -connect "${HOST}:${PORT}" -servername "${SNI}" -tls1_2 -curves X25519 2>&1 || true)
fi
...
OCSP_BLOCK=$(echo QUIT | openssl s_client -connect "${HOST}:${PORT}" -servername "${SNI}" ${OCSP_PROTO} -status 2>/dev/null \
  | sed -n '/OCSP response:/,/---/p' || true)

if [[ -z "$OCSP_BLOCK" ]]; then
  ALT_PROTO=$([[ "$PREFERRED_PROTO" == "-tls1_3" ]] && echo "-tls1_2" || echo "-tls1_3")
  OCSP_BLOCK=$(echo QUIT | openssl s_client -connect "${HOST}:${PORT}" -servername "${SNI}" ${ALT_PROTO} -status 2>/dev/null \
    | sed -n '/OCSP response:/,/---/p' || true)
fi
...
########################################
# 5) 证书信息 / 公钥长度（bit & byte）
########################################
echo -e "${BOLD}>>> 检测证书信息与公钥长度（bit / 字节）${RESET}"

# 使用优先协议获取“站点证书”（链中第一个）
CERT_TEXT=$(echo | openssl s_client -connect "${HOST}:${PORT}" -servername "${SNI}" ${PREFERRED_PROTO} 2>/dev/null \
  | openssl x509 -noout -text 2>/dev/null || true)

if [[ -z "$CERT_TEXT" ]]; then
  warn "无法获取证书（可能非 TLS 服务、握手失败，或 OpenSSL 不兼容）"
else
  # 公钥算法
  CERT_ALGO_LINE=$(echo "$CERT_TEXT" | sed -n 's/^[[:space:]]*Public Key Algorithm: \(.*\)$/\1/p' | head -n1 || true)
  [[ -n "${CERT_ALGO_LINE:-}" ]] && CERT_ALGO="$CERT_ALGO_LINE"

  # RSA: Public-Key: (2048 bit)
  CERT_BITS_LINE=$(echo "$CERT_TEXT" | sed -n 's/^[[:space:]]*Public-Key: (\([0-9]\+\) bit)$/\1/p' | head -n1 || true)

  CURVE=""
  if [[ -n "${CERT_BITS_LINE:-}" ]]; then
    CERT_BITS="${CERT_BITS_LINE} bit"
    # 按位数换算字节（向上取整，避免非8整除）
    if [[ "$CERT_BITS_LINE" =~ ^[0-9]+$ ]]; then
      local_bits="$CERT_BITS_LINE"
      local_bytes=$(( (local_bits + 7) / 8 ))
      CERT_BYTES="${local_bytes} 字节"
    fi
  else
    # ECC: 通过曲线名称大致推算位数/字节
    CURVE=$(echo "$CERT_TEXT" | sed -n 's/^[[:space:]]*ASN1 OID: \(.*\)$/\1/p' | head -n1 || true)
    if [[ -n "${CURVE:-}" ]]; then
      case "$CURVE" in
        prime256v1|secp256r1)
          CERT_BITS="约 256 bit (曲线: ${CURVE})"
          CERT_BYTES="约 32 字节"
          ;;
        secp384r1)
          CERT_BITS="约 384 bit (曲线: ${CURVE})"
          CERT_BYTES="约 48 字节"
          ;;
        secp521r1)
          CERT_BITS="约 521 bit (曲线: ${CURVE})"
          CERT_BYTES="约 65 字节"
          ;;
        *)
          CERT_BITS="曲线: ${CURVE}（具体位数视曲线而定）"
          CERT_BYTES="未知（曲线 ${CURVE}）"
          ;;
      esac
    fi
  fi
  ...
  SUBJECT_LINE=$(echo "$CERT_TEXT" | sed -n 's/^subject= *//p' | head -n1 || true)
  ISSUER_LINE=$(echo "$CERT_TEXT" | sed -n 's/^issuer= *//p' | head -n1 || true)
  NOTAFTER_LINE=$(echo "$CERT_TEXT" | sed -n 's/ *Not After *: *//p' | head -n1 || true)

  [[ -n "${SUBJECT_LINE:-}" ]]   && CERT_SUBJECT="$SUBJECT_LINE"
  [[ -n "${ISSUER_LINE:-}" ]]    && CERT_ISSUER="$ISSUER_LINE"
  [[ -n "${NOTAFTER_LINE:-}" ]]  && CERT_NOTAFTER="$NOTAFTER_LINE"

  if [[ -n "${CERT_BITS_LINE:-}" || -n "${CURVE:-}" ]]; then
    if [[ "$CERT_BYTES" != "未知" ]]; then
      ok "已获取证书公钥长度：${CERT_BITS} / ${CERT_BYTES}"
    else
      ok "已获取证书公钥长度（bit）：${CERT_BITS}"
      warn "字节数未能精确换算"
    fi
  else
    warn "未能解析证书公钥长度（可能是非常新的算法或输出格式不兼容）"
  fi

  # 打印部分证书内容以便手动检查
  echo "$CERT_TEXT" | sed -n '1,25p'
fi

echo

########################################
# 6) 统计 Certificate chain 总长度 & 证书数量
########################################
echo -e "${BOLD}>>> 统计 Certificate chain 总长度（字节）与证书数量${RESET}"

CHAIN_RAW=$(echo | openssl s_client -connect "${HOST}:${PORT}" -servername "${SNI}" ${PREFERRED_PROTO} -showcerts 2>/dev/null || true)

if [[ -z "$CHAIN_RAW" ]]; then
  warn "无法获取完整证书链（-showcerts 输出为空）"
else
  # 拆分出每一段 -----BEGIN CERTIFICATE----- ... -----END CERTIFICATE-----
  # 写入临时文件 $TMPDIR/cert_1.pem, cert_2.pem, ...
  echo "$CHAIN_RAW" | awk -v dir="$TMPDIR" '
    /BEGIN CERTIFICATE/ { cert++ }
    /BEGIN CERTIFICATE/,/END CERTIFICATE/ {
      print > (dir "/cert_" cert ".pem")
    }
  '
  ...
  for f in "$TMPDIR"/cert_*.pem; do
    [[ -e "$f" ]] || continue
    size=$(openssl x509 -in "$f" -outform der 2>/dev/null | wc -c | awk "{print \$1}")
    # 如果解析失败 size 可能为空，跳过
    [[ -z "$size" ]] && continue
    total=$(( total + size ))
    count=$(( count + 1 ))
  ...
  if (( count > 0 )); then
    CERT_CHAIN_TOTAL_BYTES="${total}"
    CERT_CHAIN_COUNT="${count}"
    echo "Certificate chain's total length:  ${total} (certs count: ${count})"
  else
    warn "未能从 -showcerts 中解析出任何证书块"
    CERT_CHAIN_TOTAL_BYTES="未知"
    CERT_CHAIN_COUNT="0"
  fi
fi
...
printf " %-26s : %s\n" "证书公钥算法"              "$CERT_ALGO"
printf " %-26s : %s\n" "证书公钥长度(bit)"         "$CERT_BITS"
printf " %-26s : %s\n" "证书公钥长度(字节)"        "$CERT_BYTES"
printf " %-26s : %s\n" "证书链总长度(字节)"        "$CERT_CHAIN_TOTAL_BYTES"
printf " %-26s : %s\n" "证书链证书数量"            "$CERT_CHAIN_COUNT"
printf " %-26s : %s\n" "证书 Subject"             "$CERT_SUBJECT"
printf " %-26s : %s\n" "证书 Issuer"              "$CERT_ISSUER"
printf " %-26s : %s\n" "证书到期时间"              "$CERT_NOTAFTER"
echo
